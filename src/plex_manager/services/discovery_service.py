...
async def home(tmdb: MetadataPort) -> HomeFeed:
    ...
    # Honesty over silence: if EVERY row fetch failed (e.g. TMDB down), surface the
    # error instead of a silently-empty home. A partial failure logs + empties that
    # one row so the rest of the home still renders.
    ...
    for (category, title), result in zip(_ROWS, results, strict=True):
        if isinstance(result, MediaPage):
            ...
            _logger.warning("discover row %r failed: %s", category, type(result).__name__)
            items = []
