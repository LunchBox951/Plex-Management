...
The implementation is synchronous because probing is a bounded local subprocess
operation and the import service already runs its blocking work off the event
loop.
"""
...
class MediaProbeResult(BaseModel):
    ...
    the path's suffix.  ``video_codec`` is the first known, non-attached-picture
    video stream's codec name.
    """
