...
async def test_import_tv_finalize_reread_locks_the_download_row_first(
    ...
    PostgreSQL is expected but unverified to re-check a blocked ``UPDATE``'s WHERE
    against the updated target tuple while still evaluating the subquery on the older
    command snapshot, so a ``download_scopes`` row the attach committed while the swap
    waited on the row lock stays invisible, the swap matches, and the late scope is
    terminalized over. No PostgreSQL-backed test exercises this; SQLite, the sole
    supported 1.0 backend, is fully covered by the existing tests. SQLite's single-writer
    behaviour cannot express that anomaly, so this pins the lock ORDER -- which is what
    makes the read observe the attach's commit -- rather than trying to reproduce the
    interleaving.
    """
    tv_root = tmp_path / "tv"
    tv_root.mkdir()
