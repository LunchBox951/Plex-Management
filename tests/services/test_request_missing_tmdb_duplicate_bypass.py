...
async def test_request_without_tmdb_id_refuses_duplicate_without_proof(engine: AsyncEngine) -> None:
    torrent_hash = "7" * 40
    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        with pytest.raises(grab_service.ClientHashOwnershipUnprovenError):
            await grab_service.grab(
                qbt, session, scored=scored(torrent_hash), request_id=request.id, tmdb_id=None
            )
        assert (
