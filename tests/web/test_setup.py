...
async def test_complete_flips_initialized_and_is_keyless(client: httpx.AsyncClient) -> None:
...
async def test_double_complete_leaves_exactly_one_intact_set_of_creds(
    ...
    second = await client.post(
        "/api/v1/setup/complete", json={**_COMPLETE_BODY, "plex_url": "http://evil.local:32400"}
    )
    assert second.status_code == 409
